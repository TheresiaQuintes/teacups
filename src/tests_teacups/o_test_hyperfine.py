...
class Test_set_up_hyperfine_tensors:
    ...
    def test_type_list(self):
        assert isinstance(self.cal.A_tensor, list)
...
class Test_create_hf_hamiltonian:
    def setup_method(self):
        ...
        self.ham_1_1 = hf.create_hf_hamiltonian([1 / 2], [[1 / 2]], [[self.A_iso_2]])
        self.ham_1_2 = hf.create_hf_hamiltonian(
            [1 / 2], [[1 / 2, 1]], [[self.A_iso_4, self.A_iso_2]]
        )
        self.ham_2_1_A = hf.create_hf_hamiltonian(
            [1 / 2, 1 / 2], [[1 / 2], []], [[self.A_iso_4], []]
        )
        self.ham_2_1_B = hf.create_hf_hamiltonian(
            [1 / 2, 1], [[], [1 / 2]], [[], [self.A_iso_2]]
        )
    ...
    def test_1_1_values(self):
        np.testing.assert_allclose(
            np.array([1 / 2, -1 / 2, -1 / 2, 1 / 2]), self.ham_1_1[0][0]
        )
    ...
    def test_1_2_value(self):
        np.testing.assert_allclose(
            np.array([2, 1, 0, 0, -1, -2, -2, -1, 0, 0, 1, 2]), self.ham_1_2[0][0]
        )
    ...
    def test_2_1_A_value_hamA(self):
        np.testing.assert_allclose(np.array([1, -1, -1, 1]), self.ham_2_1_A[0][0])
    ...
    def test_2_1_B_value_hamB(self):
        np.testing.assert_allclose(np.array([1, -1, 0, 0, -1, 1]), self.ham_2_1_B[1][0])
...
class Test_hyperfine_of_coupled_system:
    def setup_method(self):
        ...
        self.ham = hf.create_hf_hamiltonian(
            [1 / 2, 1], [[1 / 2, 1 / 2], []], [[self.A, self.A], []]
        )
        self.ham_coup = hf.hyperfine_of_coupled_system(self.ham, [4, 1], [2, 3], 6)
