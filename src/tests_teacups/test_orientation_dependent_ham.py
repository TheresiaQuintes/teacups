...
class TestTensorRotation:
    ...
    def test_type(self):
        assert isinstance(self.rotated_unit, np.ndarray)
    ...
    def test_values(self):
        num_rot_tensor = np.array(
            [
                [0.821379, -0.05376802, -0.17383894],
                [3.07396785, -0.20122401, -0.65058311],
                [-1.79474586, 0.11748527, 0.37984501],
            ]
        )
        np.testing.assert_allclose(self.rotated_tensor, num_rot_tensor, atol=1e-6)
...
class TestCreateLinearHamiltonian:
    def setup_method(self):
        a = np.arange(1, 17, dtype=np.complex64).reshape(4, 4)
        b = np.arange(17, 33, dtype=np.complex64).reshape(4, 4)
        c = np.arange(33, 49, dtype=np.complex64).reshape(4, 4)
    ...
    def test_type(self):
        assert isinstance(self.ham_z, np.ndarray)
        assert isinstance(self.ham_x, np.ndarray)
...
class TestCreateBilinearHamiltonian:
    ...
    def test_type(self):
        assert isinstance(self.ham, np.ndarray)
