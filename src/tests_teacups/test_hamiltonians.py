...
class TestMwHamiltonian:
    ...
    def test_dtype(self):
        ...
        ham_mw = ham.set_up_mw_hamiltonian(self.sys, self.exp, self.opt, self.cal)
    ...
    def test_hermitean(self):
        ...
        np.testing.assert_array_equal(
            ham_mw, np.conj(np.transpose(ham_mw, (0, 1, 3, 2)))
        )
...
class TestSetUpDoubletHamiltonian:
    def setup_method(self):
        ...
        self.cal.g_tensor = cr.create_tensor(self.sys.g, self.cal.phi, self.cal.theta)

        self.cal.ham = ham.set_up_doublet_hamiltonian(self.exp, self.opt, self.cal)
    ...
    def test_hermitean(self):
        np.testing.assert_array_equal(
            self.cal.ham, np.conj(np.transpose(self.cal.ham, (0, 1, 3, 2)))
        )
...
class TestSetUpTripletHamiltonian:
    def setup_method(self):
        ...
        self.cal.ham = ham.set_up_triplet_hamiltonian(self.exp, self.opt, self.cal)
    ...
    def test_zeeman_coupling_elements(self):
        ...
        for b in range(0, 4):
            for a in range(0, 3):
                ham_zeeman[b, a, 0, 0] = (
                    zeeman_interaction[b] * self.cal.g_tri_tensor.multirot[a, 2, 2]
                )
                ham_zeeman[b, a, 2, 2] = (
                    -zeeman_interaction[b] * self.cal.g_tri_tensor.multirot[a, 2, 2]
                )
...
class TestSetUpTripletHighFieldXyzHamiltonian:
    def setup_method(self):
        initialize_classes(self)
        ...
        self.cal.ham = ham.set_up_triplet_high_field_xyz_hamiltonian(
    ...
    def test_hermitean(self):
        np.testing.assert_allclose(
            self.cal.ham, np.conj(np.transpose(self.cal.ham, (0, 1, 3, 2))), atol=2e6
        )
    ...
    def test_basis_transformation(self):
        ...
        back_transformed = (
            vec_d @ self.cal.ham @ np.conj(np.transpose(vec_d, (0, 1, 3, 2)))
        )
        np.testing.assert_allclose(
...
class TestSetUpTripletHighFieldpnmHamiltonian:
    def setup_method(self):
        initialize_classes(self)
        ...
        self.cal.ham = ham.set_up_triplet_high_field_pnm_hamiltonian(
            self.exp, self.opt, self.cal
        )
    ...
    def test_value(self):
        ...
        np.testing.assert_allclose(ham_tri_hf.B_angle_matrix, self.cal.ham, atol=2e6)
...
class TestSetUpRpHamiltonian:
    def setup_method(self):
        ...
        self.cal.ham = ham.set_up_rp_hamiltonian(self.sys, self.exp, self.opt, self.cal)
    ...
    def test_anisotropic_g(self):
        ...
        sum_g = (
            self.cal.g1_tensor.multirot[:, 2, 2] + self.cal.g2_tensor.multirot[:, 2, 2]
        ) / 2
...
class TestSetUpTdpHamiltonianIsotrope:
    ...
    def test_zeeman(self):
        ...
        ham_tdp = ham.set_up_tdp_hamiltonian(self.sys, self.exp, self.opt, self.cal)
    ...
    def test_dipole(self):
        ...
        self.cal.D_tensor = cr.create_tensor([6, 6, 6], self.cal.phi, self.cal.theta)
        ham_tdp = ham.set_up_tdp_hamiltonian(self.sys, self.exp, self.opt, self.cal)
        np.testing.assert_allclose(ham_tdp[0, 0], ham_comp)
...
class TestSetUpTdpHamiltonianAnisotrope:
    def setup_method(self):
        ...
        cr.set_up_tensors(self.sys, self.cal)
        self.cal.ham = ham.set_up_tdp_hamiltonian(
...
class TestSetUpTdpHighFieldXyzHamiltonian:
    def setup_method(self):
        initialize_classes(self)
        ...
        cr.set_up_tensors(self.sys, self.cal)
        self.cal.ham = ham.set_up_tdp_high_field_xyz_hamiltonian(
            self.sys, self.exp, self.opt, self.cal
        )
        ...
        self.ham_zfs = mut.Multioperator(
            s_trip, self.opt.grid_points, self.exp.B_z * MU_B
        )
    ...
    def test_zfs(self):
        ...
        ham_zfs = (
            np.conj(np.transpose(self.vec_zfs, (0, 1, 3, 2)))
            @ self.ham_zfs
            @ self.vec_zfs
        )
...
class TestSetUpTdpHighFieldPnmHamiltonian:
    def setup_method(self):
        initialize_classes(self)
        ...
        cr.set_up_tensors(self.sys, self.cal)
        self.cal.ham = ham.set_up_tdp_high_field_pnm_hamiltonian(
            self.sys, self.exp, self.opt, self.cal
        )
    ...
    def test_zfs(self):
        ...
        np.testing.assert_allclose(self.cal.ham, self.ham_zfs, atol=2e-6)
    ...
    def test_zeeman_triplet(self):
        ...
        self.cal.ham = self.cal.ham - self.ham_zfs
        zeeman = self.ham_zeeman_trip.B_angle_matrix
        np.testing.assert_allclose(self.cal.ham, zeeman, atol=2e-6)
...
class TestSetUpCommutatorSuperoperator:
    ...
    def test_commutator(self):
        ...
        superop = np.kron(np.eye(3), self.hamiltonian) - np.kron(ham_adj, np.eye(3))
