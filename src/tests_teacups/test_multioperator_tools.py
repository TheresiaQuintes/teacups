...
class TestBasisTransformation:
    ...
    def test_inverse_right(self):
        ...
        np.testing.assert_allclose(c.B_angle_matrix, self.m.B_angle_matrix, atol=2e-6)
    ...
    def test_inverse_left_orthonormal(self):
        ...
        np.testing.assert_allclose(self.m.B_angle_matrix, comp)
        assert self.m.B_angle_matrix.dtype == "complex64"
    ...
    def test_orthonormal_identical(self):
        ...
        np.testing.assert_allclose(normal.B_angle_matrix, onormal.B_angle_matrix)
...
class TestBuildVector:
    def setup_method(self):
        ...
        self.o.B_angle_matrix = np.arange(2, 26, dtype=np.float32).reshape((2, 3, 2, 2))
        self.o.build_vector()
...
class TestCreateLinearOperator:
    ...
    def test_value(self):
        comp = np.array(
            [
                odh.create_linear_hamiltonian(self.g.multirot[0], self.s.matrix),
                odh.create_linear_hamiltonian(self.g.multirot[1], self.s.matrix),
                odh.create_linear_hamiltonian(self.g.multirot[2], self.s.matrix),
                odh.create_linear_hamiltonian(self.g.multirot[3], self.s.matrix),
                odh.create_linear_hamiltonian(self.g.multirot[4], self.s.matrix),
            ]
        )
    ...
    def test_hermitean(self):
        np.testing.assert_array_equal(
            self.h.angle_matrix, np.conj(np.transpose(self.h.angle_matrix, (0, 2, 1)))
        )
...
class TestZeemanCoupling:
    ...
    def test_value(self):
        np.testing.assert_allclose(self.h.B_angle_matrix[0], self.comp.angle_matrix * 1)
        np.testing.assert_allclose(self.h.B_angle_matrix[1], self.comp.angle_matrix * 2)
        np.testing.assert_allclose(self.h.B_angle_matrix[2], self.comp.angle_matrix * 3)
        np.testing.assert_allclose(self.h.B_angle_matrix[3], self.comp.angle_matrix * 4)
